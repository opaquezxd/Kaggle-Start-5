...
def train_test(num_rows=None):
    ...
    day_df=pd.read_csv(
        'F:\\zxd\\elo-merchant-category-recommendation\\Cardreferencedate.csv',
        index_col=['card_id'], nrows=num_rows)
    train_df=pd.merge(train_df, day_df, left_index=True, right_index=True, how='left')
    test_df = pd.merge(test_df, day_df, left_index=True, right_index=True, how='left')

    train_df = pd.get_dummies(train_df, columns=['category_month_lag'] )
    test_df = pd.get_dummies(test_df, columns=['category_month_lag'])
    ...
    df['elapsed_time'] = (pd.to_datetime(df['reference_date']) - df['first_active_month']).dt.days
...
def main(debug=False,nrow=None):
    ...
    train_df.to_csv("F:\\zxd\\elo-merchant-category-recommendation\\train_aggregate26_10.csv", index=False)
    test_df.to_csv("F:\\zxd\\elo-merchant-category-recommendation\\test_aggregate26_10.csv", index=False)
    del df
    gc.collect()
    ...
    kfold_lightgbm(important_features,train_df, test_df, num_folds=11, stratified=False, debug=debug)
