...
def train_test(num_rows=None):
    ...
    #train_df.loc[train_df['outliers'] == 1, 'target'] = train_df[train_df['outliers'] ==0]['target'].mean()
...
def historical_transactions(num_rows=None):
    ...
    hist_df['month_diff'] = ((pd.to_datetime('2018-02-1') - hist_df['purchase_date']).dt.days)//30
...
def new_merchant_transactions(num_rows=None):
    ...
    new_merchant_df['month_diff'] = ((pd.to_datetime('2018-02-1') - new_merchant_df['purchase_date']).dt.days)//30
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    test_df[['card_id', 'target']].to_csv("submission201922_3.csv", index=False)

def main(debug=False,rowNum=None):
    df = train_test(rowNum)
    df = pd.merge(df, historical_transactions(rowNum), on='card_id', how='outer')
    df = pd.merge(df, new_merchant_transactions(rowNum), on='card_id', how='outer')
    df = additional_features(df)
    train_df = df[df['target'].notnull()]
    test_df = df[df['target'].isnull()]
    #train_df.to_csv("F:\\zxd\\elo-merchant-category-recommendation\\train_aggregate.csv", index=False)
    del df
    gc.collect()
